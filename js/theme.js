    ...
    initSearch() {
        ...
        const initAutosearch = () => {
            const autosearch = autocomplete(`#search-input-${suffix}`, {
                    ...
                    if (searchConfig.type === 'lunr') {
                        ...
                        loadPagefind().then(pagefind => {
                            pagefind.debouncedSearch(query, {}, 300).then(async query_result => {
                                const finalResults = [];
                                if (!query_result || !query_result.results) {
                                    finish(finalResults);
                                    return;
                                }
                                let resultCount = 0;
                                ...
                                for (let i = 0; i < results.length && finalResults.length < maxResultLength; i++) {
                                    try {
                                        ...
                                        if (/^\/\d{6}\//.test(result.url) && !/\.html?$/.test(result.url)) {
                                            ...
                                            for (let sub_i = 0; sub_i < result.sub_results.length; sub_i++) {
                                                const sub_result = result.sub_results[sub_i];
                                                finalResults.push({
                                                    uri: sub_result.url,
                                                    title: sub_result.title,
                                                    date: "",
                                                    context: sub_result.excerpt,
                                                    sub: true,
                                                });
                                            }
                ...
                templates: {
                    suggestion: ({ title, date, context, sub }) => {
                        if (!sub) {
                            ...
                            return `<div style="padding-left: 4em;"><div><span class="suggestion-title suggestion-sub">${title}</span></div><div class="suggestion-context">${context}</div></div>`;
                        }
                    },
